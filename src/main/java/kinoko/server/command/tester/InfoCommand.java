...
public final class InfoCommand {
    ...
    public static void info(User user, String[] args) {
        ...
        // Basic stats
        user.systemMessage("HP : %d / %d, MP : %d / %d", user.getHp(), user.getMaxHp(), user.getMp(), user.getMaxMp());
        user.systemMessage("STR : %d, DEX : %d, INT : %d, LUK : %d", stats.getStr(), stats.getDex(), stats.getInt(), stats.getLuk());
        user.systemMessage("AP : %d", charStats.getAp());
        user.systemMessage("SP : %s", charStats.getSp().getMap());
        user.systemMessage("Damage : %d ~ %d", (int) CalcDamage.calcDamageMin(user), (int) CalcDamage.calcDamageMax(user));
        user.systemMessage("Field ID : %d (%s)", field.getFieldId(), field.getFieldType());

        // Foothold below
        final String footholdBelow = field.getFootholdBelow(user.getX(), user.getY())
                .map(fh -> String.valueOf(fh.getSn()))
                .orElse("unk");
        user.systemMessage("  x : %d, y : %d, fh : %d (%s)", user.getX(), user.getY(), user.getFoothold(), footholdBelow);
        ...
        if (nearestPortal != null && Util.distance(user.getX(), user.getY(), nearestPortal.getX(), nearestPortal.getY()) < 200) {
            user.systemMessage("Portal name : %s (%d)", nearestPortal.getPortalName(), nearestPortal.getPortalId());
            user.systemMessage("  x : %d, y : %d, script : %s", nearestPortal.getX(), nearestPortal.getY(), nearestPortal.getScript());
        }
        ...
        user.getNearestObject(field.getMobPool().getInsideRect(user.getRelativeRect(detectRect)))
                .ifPresent(mob -> {
                    user.systemMessage(mob.toString());
                    user.systemMessage("  Controller : %s", mob.getController().getCharacterName());
                });

        // Nearest NPC
        user.getNearestObject(field.getNpcPool().getInsideRect(user.getRelativeRect(detectRect)))
                .ifPresent(npc -> user.systemMessage(npc.toString()));

        // Nearest Reactor
        user.getNearestObject(field.getReactorPool().getInsideRect(user.getRelativeRect(detectRect)))
                .ifPresent(reactor -> user.systemMessage(reactor.toString()));
    }
}
